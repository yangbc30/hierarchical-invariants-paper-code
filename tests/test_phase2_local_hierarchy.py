...
def test_scope_conflict_raises_error():
    ...
    with pytest.raises(ValueError):
        _ = rho.invariant.I_exact(1, sector=(2, 1), copy=((2, 1), 0))
...
def test_copy_local_jordan_absorption_and_invariance_basis_dependent():
    sys = PhotonicSystem(m_ext=2, n_particles=3, rng=np.random.default_rng(12))
    copy_scope = ((2, 1), 0)
    rho = sys.state.random_sector((2, 1)).project_copy(*copy_scope)

    proj = rho.project_jordan(2, kind="cumulative", copy=copy_scope)
    Qa = sys.copy_projector(*copy_scope)
    ...
    for order in range(3):
        in_exact = rho.invariant.I_exact(order, copy=copy_scope)
        out_exact = rho_out.invariant.I_exact(order, copy=copy_scope)
        assert abs(in_exact - out_exact) < 1e-8

        in_cum = rho.invariant.I_cumulative(order, copy=copy_scope)
        out_cum = rho_out.invariant.I_cumulative(order, copy=copy_scope)
        assert abs(in_cum - out_cum) < 1e-8

    report = rho_out.analyze(max_order=2, include_sectors=True, copy=copy_scope)
